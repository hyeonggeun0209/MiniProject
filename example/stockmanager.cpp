   ...
   if(!file.fail()) {
       while(!file.eof()) {
           ...
           if(row.size()) {
               ...
               Stock* c = new Stock(id, atoi(row[1].c_str()), atoi(row[2].c_str()), row[3]);
...
void StockManager::inputStock( )
{
    ...
    int id = makeId( );
    Stock* c = new Stock(id, atoi(pid.c_str()), atoi(quantity.c_str()), section);
    StockList.insert( { id, c } );
}
...
{
    Stock* c = search(key);
    cout << "  ID  |    Product ID  |    Section Name   |   Quantity "<< endl;
    cout << setw(5) << setfill('0') << right << c->id() << " | " << left;
    cout << setw(12) << setfill(' ') << c->getP_id() << " | ";
