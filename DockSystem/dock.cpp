...
Dock::Dock(int id, const string& vehicleNumber, int quantity, const string& itemType, const string& status, time_t entryTime, time_t exitTime)
    : m_id(id), // 도크의 ID를 설정합니다.
    m_vehicleNumber(vehicleNumber), // 차량 번호를 설정합니다.
    m_quantity(quantity), // 물품 수량을 설정합니다.
    m_itemType(itemType), // 물품 종류를 설정합니다.
    m_status(status), // 도크의 현재 상태를 설정합니다.
...
{
    return m_quantity; // 물품 수량을 반환합니다.
}

// 물품 수량을 설정하는 메서드
void Dock::setQuantity(int quantity)
{
    m_quantity = quantity; // 주어진 수량으로 도크의 물품 수량을 설정합니다.
}
...
{
    return m_status; // 도크의 상태를 반환합니다.
}

// 도크 상태를 설정하는 메서드
void Dock::setStatus(const string& status)
{
    m_status = status; // 주어진 상태로 도크의 상태를 설정합니다.
}
